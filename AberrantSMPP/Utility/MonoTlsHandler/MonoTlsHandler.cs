...
namespace DotNetty.Handlers.Tls
{
    ...
    public sealed partial class MonoTlsHandler : ByteToMessageDecoder
    {
        ...
        public MonoTlsHandler(TlsSettings settings)
            : this(stream => new SslStream(stream, true), settings)
        {
        }

        public MonoTlsHandler(Func<Stream, SslStream> sslStreamFactory, TlsSettings settings)
        {
        ...
        public static MonoTlsHandler Client(string targetHost) => new MonoTlsHandler(new MonoClientTlsSettings(targetHost));

        public static MonoTlsHandler Client(string targetHost, X509Certificate clientCertificate) => new MonoTlsHandler(new MonoClientTlsSettings(targetHost, new List<X509Certificate>{ clientCertificate }));
 
        public static MonoTlsHandler Server(X509Certificate certificate) => new MonoTlsHandler(new ServerTlsSettings(certificate));
        ...
        static void HandleHandshakeCompletedInternal(Task task, MonoTlsHandler self)
        {
            switch (task.Status)
            {
        ...
        protected override void Decode(IChannelHandlerContext context, IByteBuffer input, List<object> output)
        {
            ...
            while (totalLength < MonoTlsUtils.MAX_ENCRYPTED_PACKET_LENGTH)
            {
                int readableBytes = endOffset - offset;
                if (readableBytes < MonoTlsUtils.SSL_RECORD_HEADER_LENGTH)
                {
                    break;
                }

                int encryptedPacketLength = MonoTlsUtils.GetEncryptedPacketLength(input, offset, out byte contentType);
                ...
                if (newTotalLength > MonoTlsUtils.MAX_ENCRYPTED_PACKET_LENGTH)
                {
                    // Don't read too much.
                    break;
                }
            ...
            try
            {
                ...
                if (packetInfos.Count > 0)
                {
                    ...
                    while (!this.EnsureAuthenticated())
                    {
                        ...
                        if (type == MonoTlsUtils.SSL_CONTENT_TYPE_APPLICATION_DATA)
                        {
        ...
        bool EnsureAuthenticated()
        {
            ...
            if (!oldState.HasAny(TlsHandlerState.AuthenticationStarted))
            {
                ...
                {
                    var clientSettings = (MonoClientTlsSettings)this.settings;
        ...
        void FinishWrap(byte[] buffer, int offset, int count)
        {
            ...
            this.lastContextWriteTask =
                (this.flushMode == FlushMode.ForceFlush) ?
                    this.capturedContext.WriteAndFlushAsync(output)
                    : this.capturedContext.WriteAsync(output);
        }
